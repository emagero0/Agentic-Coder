...
Every response MUST follow this two-section structure. Section 1 is ALWAYS present — it tells the caller whether ExternalScout is needed.

### Section 1: ExternalScout Recommendation (ALWAYS present, placed first)

```markdown
## ⚠️ ExternalScout Required

**Library**: `[name]` or `None`
**Status**: `No internal context — fetch required` | `Internal coverage exists` | `Not applicable`
**Action**: `Invoke ExternalScout before implementing` | `Use internal context files` | `None`
```

**Rules**:
- `No internal context — fetch required` → library detected, no internal coverage → caller MUST invoke ExternalScout
- `Internal coverage exists` → library has internal context files in results below → use those instead
- `Not applicable` → no external library involved → no action needed

### Section 2: Priority-Ranked Context Files
